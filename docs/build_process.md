...
3. `usecase.yaml` を作る。
4. `uv run usecaseapi manifest validate usecase.yaml` を実行する。
5. `uv run usecaseapi manifest scaffold usecase.yaml --root . --dry-run` で生成予定を出す。
...
```bash
uv run usecaseapi manifest scaffold usecase.yaml --root . --dry-run
```
...
```bash
uv run usecaseapi manifest scaffold usecase.yaml --root .
```
